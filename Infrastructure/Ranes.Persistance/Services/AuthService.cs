...
using Ranes.Application.DTOs;
using Ranes.Application.Exceptions;
...
namespace Ranes.Persistence.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<bool> Auth(PageName pageName, string token, OperationType operationType)
        {
            var newToken = await RefreshTokenLoginAsync(token);
            if (newToken == null) return false;
            var handler = new JsonWebTokenHandler();
            var jwt = new JwtSecurityTokenHandler().ReadJwtToken(newToken.AccessToken);
            string[] roles = jwt.Claims.First(c => c.Type == "Role").Value.Split(',');
            //roleService'e method yazılacak pageName ve roles'e göre db de search yapılacak buradan da true veya false dönecek
